(() => {
    ...
    function getUserId() { return parseInt(localStorage.getItem(USER_ID_KEY) || "0", 10); }

    function setToken(t) { if (t) localStorage.setItem(TOKEN_KEY, t); }
    function setRole(role) { if (role) localStorage.setItem(ROLE_KEY, role); }
    function setEmail(email) { if (email) localStorage.setItem(EMAIL_KEY, email); }
    function setUserId(id) { if (id != null) localStorage.setItem(USER_ID_KEY, String(id)); }
    ...
    async function initNavbar() {
        ...
        if (cachedRole) setNavbarLoggedIn(cachedRole, cachedEmail);
        else setNavbarLoggedIn("user", cachedEmail);
